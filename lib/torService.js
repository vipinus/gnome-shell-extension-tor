...
// Prefer the multi-instance unit; the meta unit ExecStart=/bin/true so Start
// on it is a no-op. Callers may override via params.
const DEFAULT_UNIT = 'tor@default.service';
...
export const TorService = GObject.registerClass({
    ...
    async _jobCall(method) {
        ...
        // ALLOW_INTERACTIVE_AUTHORIZATION lets polkit prompt the active session
        // for credentials (AUTH_ADMIN_KEEP in our rule caches for the session).
        const ret = await this._call(
