...
//
// Always system bus — the unit runs as root (needs nft NAT + ip rule, both
// root-only) and creates the TUN + tweaks policy routing. Our installed
// polkit rule turns the start/stop verbs into a passwordless YES for active
// local users, so the tile UX stays one-click.
